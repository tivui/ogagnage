...
import boto3
from app.config import S3_BUCKET, S3_KEY, S3_SECRET, SQLALCHEMY_DATABASE_URI
s3 = boto3.client('s3',
...
def afficher_form_2():
    ...
    ogagnagedb.update_infos(reponse,env,SQLALCHEMY_DATABASE_URI)
...
def home_admin_ajouté():
    ...
    ogagnagedb.update_filepath(filename,env,SQLALCHEMY_DATABASE_URI)
...
def supp_admin():
    env=app.config['ENV']
    liste_db=ogagnagedb.scan_complet(env,SQLALCHEMY_DATABASE_URI)
...
def valid_supp():
    ...
    ogagnagedb.delete_carte(id_carte,env,SQLALCHEMY_DATABASE_URI)
    liste_db=ogagnagedb.scan_complet(env,SQLALCHEMY_DATABASE_URI)
    return render_template("admin/supp.html",liste_db=liste_db)
